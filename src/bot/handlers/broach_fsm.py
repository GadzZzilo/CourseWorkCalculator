...
async def broach_example_command(message: types.Message):
    """Sends an example in the form of a photo"""
    with open("bot/static/broach_preview.png", "rb") as photo:
        await message.reply_photo(photo, "Пример расчета протяжки")
...
async def broach_data_command(message: types.Message):
    ...
    await message.answer(
        ...
        reply_markup=get_kb("Отменить", "Пример расчета")
    )
...
def register_broach_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(broach_example_command,
