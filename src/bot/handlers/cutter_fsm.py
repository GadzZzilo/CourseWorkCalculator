...
async def cut_type_command(message: types.Message):
    ...
    await message.answer(
        "Выберите тип резца",
        reply_markup=get_kb("Круглый", "Призматический", "Пример расчета", "Отменить")
    )
...
async def cutter_example_command(message: types.Message):
    with open('bot/static/circle_preview.png', 'rb') as photo_circle, \
...
def register_cutter_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(cutter_example_command,
