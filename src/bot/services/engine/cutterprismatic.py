...
class CourseWorkPartCutterPrismatic(BaseCourseWorkPart):
    ...
    def __part_3(self):
        ...
        if m.ceil(self.section_count) >= 9:
            self.step = 10
            self.section_count = round(self.delta_work / self.step, 2)
    ...
    def __part_4(self):
        image = ("Рисунок представленный ниже является лишь схематической версией настоящего чертежа\n"
                 "Однако с помощью него можно понять каким образом и с какой стороны начинают считаться\n"
                 "точки вашего варианта\n\n")
        if self.points_count == 8:
            image += '\n   --------------------------------------------------------------------------------------------------------------------\n' + \
                    '            ║            ║             ║                                                              ║       ║  ║\n' * 12 + \
                    '            ║            ║             ║                                                              ║       ║  /\n' + \
                    '            ║            ║             ║                                                              ║       ║ /\n' + \
